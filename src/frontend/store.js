...
import components from '@front-new/views/components/module'
...
function commitMutation ({commit, state }, entry) {
  bridge.send('vuex:mutation', entry)
}
...
export function createStore () {
  ...
  if (module.hot) {
    module.hot.accept([
      '@front-new/views/components/module',
      ...
      try {
        store.hotUpdate({
          modules: {
            components: require('@front-new/views/components/module').default,
  ...
  store.oldcommit = store.commit
  store.commit = (...args) => {
    console.warn(...args)
    store.oldcommit(...args)
  }
